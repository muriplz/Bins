    ...
    >
      ...
      <div v-else>
        <div class="profile-info">
          ...
          <span class="user-level">Level: {{ Store.getUser().level || 1 }}</span>
          <span class="user-points">Points: {{ Store.getUser().points || 0 }}</span>
        </div>
        <a href="javascript:void(0)" class="dropdown-item" @click="emitEvent('showProfile')">View Profile</a>
        <a href="javascript:void(0)" class="dropdown-item" @click="emitEvent('logout')">Logout</a>
      </div>
    </div>
  </Transition>
</template>
